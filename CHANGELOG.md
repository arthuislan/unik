# Changelog

<<<<<<test-v1.1.0
## v1.3.3 — Service Selector Translation Hotfix
=======
## v1.2.4 — Service Selector Update

- Added `Airbnb` to the Service Needed selector.
- Added `Personal Organizer` to the Service Needed selector.
- Preserved approved layout, Resend integration, Cloudflare Pages Functions and email templates.
- Updated cache query strings to `v=1.2.4`.

## v1.2.3 — Email CTA Removal & Dark Mode Text Refinement
>>>>>> main

### Fixed

- Added proper multilingual labels for the new service selector options in English, Spanish and Brazilian Portuguese.
- Fixed `Airbnb Cleaner` and `Home Organization & Personal Organizer` so they no longer appear only in English when the site language changes.
- Updated cache references to `v=1.3.3`.

### Preserved

- v1.3.1 email dark-mode compatibility hotfix.
- v1.3.1 deliverability-related email improvements.
- v1.3.2 service selector additions.
- Cloudflare Pages Function `/api/contact`.
- Resend integration.
- Website layout and CSS.

### Audit

- JavaScript syntax checked.
- Function syntax checked.
- No CSS changes.
- No backend email logic changes.

## v1.3.2 — Service Selector Refinement

### Changed

- Updated the service selector option from `Personal Organizer` to `Home Organization & Personal Organizer`.
- Updated the Airbnb service option to `Airbnb Cleaner`.
- Updated cache references to `v=1.3.2`.

### Preserved

- v1.3.1 email dark-mode compatibility hotfix.
- v1.3.1 deliverability-related email improvements.
- Cloudflare Pages Function `/api/contact`.
- Resend integration.
- Website layout and CSS.

### Audit

- JavaScript syntax checked.
- Function syntax checked.
- No CSS changes.
- No backend email logic changes.
